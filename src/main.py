...
from date_preprocessing import HospitalRequest
...
class BloodDashboard(tk.Tk):
    ...
    def _refresh_stats(self):
        ...
        for card_frame, val in zip(
            self.winfo_children()[1].winfo_children()[1].winfo_children()[0].winfo_children(),
            [str(total), str(types), str(banks), str(soon)]
        ):
            pass  # We update via StringVar labels in the stat cards

        # Simpler: just update the StringVars
        self.stat_total.set(str(total))
        self.stat_types.set(str(types))
        self.stat_banks.set(str(banks))
        self.stat_expiry.set(str(soon))

        # Rebuild stat labels text (they aren't dynamic by default above)
        # We handle this by re-calling pack after update — but we stored StringVars so:
        # The big labels need to be tk.Label with textvariable. Let's patch:
        # (they were made with make_label which doesn't use textvariable)
        # Fix: update label text on refresh by walking widgets — done below
    ...
    def _submit(self):
        ...
        # Reload table to reflect new inventory
